...
func TestParseEncryptedFormat(t *testing.T) {
 ...
 ciphertext, _ := crypto.Encrypt(key, []byte(plaintext))
	...
	// 构建格式字符串
	formatted := crypto.BuildEncryptedFormat(ciphertext)

	// 解析格式字符串
	parsed, err := crypto.ParseEncryptedFormat(formatted)
	if err != nil {
		t.Fatalf("解析失败: %s", err)
	}

	// 验证解析结果
	if parsed.Prefix != crypto.FormatPrefix {
		t.Fatalf("前缀错误: got %s, want %s", parsed.Prefix, crypto.FormatPrefix)
	}
	if parsed.Version != crypto.FormatVersion {
		t.Fatalf("版本错误: got %s, want %s", parsed.Version, crypto.FormatVersion)
	}
	if len(parsed.Salt) != crypto.SaltSize {
		t.Fatalf("盐值长度错误: got %d, want %d", len(parsed.Salt), crypto.SaltSize)
	}
...
func TestParseInvalidFormat(t *testing.T) {
	...
	for _, test := range tests {
		_, err := crypto.ParseEncryptedFormat(test)
		if err == nil {
...
func TestBuildEncryptedFormat(t *testing.T) {
 ...
 ciphertext, _ := crypto.Encrypt(key, []byte(plaintext))
	...
	// 验证格式字符串
	if !crypto.IsEncryptedPassword(formatted) {
...
func TestIsEncryptedPassword(t *testing.T) {
	...
	for _, test := range tests {
		result := crypto.IsEncryptedPassword(test.password)
...
func TestGetPasswordType(t *testing.T) {
	...
	for _, test := range tests {
		result := crypto.GetPasswordType(test.password)
		if result != test.expected {
...
func TestFormatRoundTrip(t *testing.T) {
 ...
 ciphertext, _ := crypto.Encrypt(key, []byte(plaintext))
	...
	// 解析格式
	parsed, _ := crypto.ParseEncryptedFormat(formatted)
