...
func TestDecryptInvalidCiphertext(t *testing.T) {
	...
	// 密文长度不是8的倍数
	_, err = Decrypt(key, []byte("12345678abcdefghij")) // 18字节，不是8的倍数
	if err != ErrInvalidCiphertext {
		t.Fatalf("预期 ErrInvalidCiphertext, got: %s", err)
	}
}
...
func TestDecryptWrongKey(t *testing.T) {
 ...
 plaintext := "mySecretPassword"
	...
	// 用 key1 加密
 ciphertext, _ := Encrypt(key1, []byte(plaintext))
...
func TestSamePasswordDifferentCiphertext(t *testing.T) {
 ...
 ct1, _ := Encrypt(key, []byte(plaintext))
	...
	// 但两次解密结果都应该一致
	dec1, _ := Decrypt(key, ct1)
	dec2, _ := Decrypt(key, ct2)
